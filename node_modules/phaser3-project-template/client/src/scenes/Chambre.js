class Chambre extends Phaser.Scene {
  ...
  create() {
    ...
    this.autre = this.physics.add.existing( new Autre(this, 1050, 440) );
    this.bg = this.physics.add.image(750, 350, 'bg');
     this.mshape﻿.fillStyle(0xffff00, 1);
     this.mshape.﻿fillRoundedRect(32, 22, 700, 10, 5);
    this.physics.enable(this.mshape);
    ...
    // });
    this.fix = this.physics.add.sprite(750, 447, "perso");
    // this.fix.scale = 0.30;
    this.fix.scale = 0.38;
    this.anims.create({
      key: "perso_anim",
  ...
  this.enemies.add(this.fix);
  this.enemies.add(this.autre);

  this.physics.add.overlap(this.fix, this.bg, this.rapelle, null, this);
  this.physics.add.overlap(this.fix, this.enemies, this.hurtplayer, null, this);

  }

  rapelle(joueur, autre) {

    autre.disableBody(true,true);

  }

  hurtplayer(joueur, autre) {
    // this.resetShipPos(autre);
    joueur.x = config.width / 2 - 8;
    joueur.y = config.height - 64;
  }
